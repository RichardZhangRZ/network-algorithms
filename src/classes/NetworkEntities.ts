  ...
  updateDistanceVector(incomingPacket: DVPacket): ChangeStatus {
    ...
    for (const destination of this.distanceVector.keys()) {
      ...
      for (const [neighbor, dv] of this.neighboringDistanceVectors.entries()) {
        const possibleDist 
            = (dv.get(destination) ?? Infinity) + (this.localLinkState.get(neighbor) ?? Infinity);
...
class DVPacket {
  ...
  constructor(source: DVRouter, destination: DVRouter, distanceVectorInfo: DistanceVector) {
    this.source = source;
